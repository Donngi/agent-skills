...
2. **ドメイン層に閉じる**。ユースケース・アプリケーションサービス・コントローラ・DTO・プレゼンテーション・インフラは図に載せない。これはプロンプト上の注意ではなく機械的なゲートで、スキーマの白リストと禁止語彙チェックが構造的に拒否する（[references/purity-rules.md](references/purity-rules.md)）
